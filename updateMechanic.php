    ...
    if (mysqli_query($conn, $sql2)) {
      ...
      if(mysqli_query($conn, $sql1)){
        ...
        if(mysqli_query($conn, $sql2)){
          ...
          $d = "SELECT $oldM FROM appointments_per_day WHERE Date = '$changedDate'"; 
          if(mysqli_query($conn, $d)){
            $result_array =mysqli_fetch_array(mysqli_query($conn,$d));
            if($result_array[0] > 0){
              $result_array[0] = $result_array[0]-1;
            }
            $d2 = "UPDATE appointments_per_day SET $oldM = $result_array[0] WHERE Date = '$changedDate'";
            if(mysqli_query($conn,$d2)){
              $query = "UPDATE client SET Mechanic = '$mechanics' WHERE id = '$id'";
